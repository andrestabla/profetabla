...
import { Briefcase, Plus, Users, ArrowRight, Trophy, AlertCircle } from 'lucide-react';
...
export async function FilteredProjectList({
    type,
    title,
    description,
    createLink,
                ...
                {projects.length === 0 && (
                    <div className="col-span-full py-20 text-center bg-white rounded-2xl border-2 border-dashed border-slate-200">
                        <div className="bg-slate-50 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                            {type === 'PROJECT' ? <Briefcase className="w-8 h-8 text-slate-300" /> :
                                type === 'CHALLENGE' ? <Trophy className="w-8 h-8 text-slate-300" /> :
                                    <AlertCircle className="w-8 h-8 text-slate-300" />}
                        </div>
                        <h3 className="text-xl font-bold text-slate-800 mb-2">{emptyMessage}</h3>
                        <p className="text-slate-500 mb-8 max-w-sm mx-auto">{emptySubMessage}</p>
                        <Link
                            href={createLink}
                            className="inline-flex items-center gap-2 text-blue-600 font-bold hover:underline"
                        >
                            {createLabel} <ArrowRight className="w-4 h-4" />
                        </Link>
                    </div>
                )}
            </div>
